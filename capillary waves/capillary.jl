...
$$F(z) = \sum^{N-1}_{n=0} u_n cos(2 \pi n z)$$

with mesh points:

$$z_I = \frac{I - 1}{2N -2}, \ \ \ \  I = 1, \cdots, N$$

Satisfying the free surface condition at mesh points gives $N$ equations. Fixing the steepness of the wave profile by requiring

$$F(z=0) - F(z=1/2) = s$$

gives us an extra equation $\implies$ $N+1$ equations for $N+1$ unknowns ($F(z)$ and $B^*$)."

# ╔═╡ bf262e7a-25bf-400e-8ff4-55199f79e69e
md"### Solve for one wave profile"

# ╔═╡ 696cddf2-9627-43df-9f0c-91ce29f025f5
md"###### defining the system of equations
...
$$[B^*, a_0, a_1, a_2, \cdots] = [5.8, 0.17, 0.09, 0, \cdots]$$"
...
function finDiff1D(i, order, y, domain)
	...
	if i == 1 # forward difference derivatives (for first point)
		if order == 1
			derivative = (y[i+1] - y[i])./h
		elseif order == 2
			derivative = (y[i+2] - 2*y[i+1] + y[i])./(h^2)
		else
			throw(ArgumentError("invalid derivative order requested"))
		end
		
	elseif i == N # backward difference derivatives (for last point)
		if order == 1
			derivative = (y[i] - y[i-1])./h
		elseif order == 2
			derivative = (y[i] - 2*y[i-1] + y[i-2])./(h^2)
		else
			throw(ArgumentError("invalid derivative order requested"))
		end

	else  # centered derivatives (for intermediate points)
		if order == 1
			derivative = (y[i+1] - y[i-1])./(2*h)
		elseif order == 2
			derivative = (y[i+1] + y[i-1] - 2*y[i])./(h^2)
		else
			throw(ArgumentError("invalid derivative order requested"))
		end
...
# ╔═╡ 9cf4fa95-54b6-4a71-9f56-0efd94a06c1d
begin
	z = collect(range(0,2*π,9))
	y = sin.(z)
end

# ╔═╡ 5cfecbe6-cd9a-4ada-9dfa-89ee1e403974
begin
	diff = zeros(length(z))
	for i = 1:length(z)
		diff[i] = finDiff1D(i, 1, y, z)
	end
end

# ╔═╡ e9704106-9b79-42cf-bc17-9d11835fe687
begin
	plot(z,y, label="sin")
	plot!(z, cos.(z), label="cos")
	plot!(z,diff, label="approx")
end

# ╔═╡ 26da9d1c-2c98-439e-b068-b27fb63aa4e2
z[length(z)]

# ╔═╡ 1764805e-e210-4f3d-bfad-0fb52700db3d
begin
