...
# ╔═╡ df5bc634-0f5d-44f9-9a12-21c0ad2b523b
begin
	using Plots
	using LaTeXStrings
...
# ╔═╡ 07690090-e4fe-11ec-2e94-63ce10836175
md"## Solitary Waves on a Ferrofluid Jet

The propagation of axisymmetric solitary waves on the surface of an otherwise cylindrical ferrofluid jet subjected to a magnetic field is investigated


"

# ╔═╡ 8c6def23-031d-4050-a2b9-d6c6ac67a339
md"### Dispersion relation"

# ╔═╡ a3228d1f-5665-4ed0-86e0-ef0293b305f9
function waveSpeed(k, b, B)

	# define components
	one = 1 ./ k
	
	two_top = besseli.(1,k).*besselk.(1,k.*b) .- besseli.(1,k.*b).*besselk.(1,k)
	two_bottom = besseli.(1,k.*b).*besselk.(0,k) .+ besseli.(0,k).*besselk.(1,k.*b)
	two = two_top ./ two_bottom

	three = k.^2 .- 1 .+ B

	# define overall
	c = sqrt.(one .* two .* three)

	return c
end

# ╔═╡ ded76e45-583a-4147-80c0-224ee30f959b
begin
	k = collect(range(0.001, 14, 1000))
	B = collect(range(1,30,5))
	b = 0.0001

	plot(k, waveSpeed(k, b, B[1]), label="B = $(B[1])", legend = :bottomright)
	
	for i = 2:length(B)
		plot!(k, waveSpeed(k, b, B[i]), label="B = $(B[i])")
	end
	ylabel!(L"c")
	xlabel!(L"k")
end

# ╔═╡ 68224c3c-ae13-4d3e-94bd-f3af0048ceb2
md"### Static solutions"

# ╔═╡ 902d2f13-71c3-464d-96d6-382212d4d93c
md"define the conservative dynamical system (eq. 4.2)"

# ╔═╡ 7fdce6fd-1d97-4777-bea5-fa3ea180164a
function system!(dy, y, B, x)
	dy[1] = sin(y[2])
	dy[2] = B/2 - 1 + cos(y[2])/y[1] - B/(2 * y[1]^2)
end

# ╔═╡ 53d0d056-4f87-4483-a5c4-f2d3f94826f7
md"solve the system for B = 1.25 (using the Runge-Kutta method)"
...
begin
	...
	problem = ODEProblem(system!, y0, xspan, 1.25)
	solution = solve(problem, RK4())

	# plot
	plot(solution, vars=(0,1), legend=false)
	xlabel!(L"x")
	ylabel!(L"S")
end

# ╔═╡ 2b813643-c4ef-41c2-8dfa-06604c4f072c
md"solve for B = 30"
...
begin
	...
	solution2 = solve(problem2, RK4())

	# plot
	plot(solution2, vars=(0,1), legend=false)
	xlabel!(L"x")
	ylabel!(L"S")
end

# ╔═╡ d48bca5b-9f6b-4ae0-bb72-b54aab33c03b


# ╔═╡ 5208e098-6ded-472a-9888-aff133a603da
md"#### Testing with Lorenz system"

# ╔═╡ cfca1c97-1b88-4fcd-b6f7-e539e440346f
...
# ╔═╡ 664b2c80-4bc0-4a39-a93a-6ea7bcdcf064
begin
	u0 = [1.0;0.0;0.0]
	tspan = (0.0,100.0)
...
git-tree-sha1 = "30bf86a56ff44ce4a44b5d3b32ea720d658c6b86"
